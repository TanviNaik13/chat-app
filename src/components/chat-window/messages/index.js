...
function shoudlScrollToBottom(node, threshold = 30) {
  const percentage =
...
const Messages = () => {
  ...
  const onLoadMore = useCallback(() => {
    const node = selfRef.current;
    const oldHeight = node.scrollTop;
    ...
    setTimeout(() => {
      ...
      node.scrollTop = newHeight - oldHeight + node.scrollTop;
    }, 200);
  }, [loadMessages, limit]);
